...
Detailed prompt templates are in `./commands/`. Invoke by describing the workflow (e.g., "do a diff review", "generate a diagram of the auth flow", "create slides for the project recap").
...
**Output location:** Write to `~/diagrams/`. Use a descriptive filename based on content: `modem-architecture.html`, `pipeline-flow.html`, `schema-overview.html`. Create the directory if it doesn't exist. The directory persists across sessions.
